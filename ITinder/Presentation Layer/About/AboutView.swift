...
class AboutView: UIView {
    ...
    let toAuthenticationStackView: UIStackView = {
        let toAuthenticationStackView = UIStackView()
        toAuthenticationStackView.translatesAutoresizingMaskIntoConstraints = false
        toAuthenticationStackView.alignment = .center
        toAuthenticationStackView.axis = .horizontal
        toAuthenticationStackView.spacing = UIScreen.main.bounds.width * 0.02

        return toAuthenticationStackView
    }()

    func toAuthenticationStackViewConstraints() {
        NSLayoutConstraint.activate([
            toAuthenticationStackView.topAnchor.constraint(equalTo: startButton.bottomAnchor, constant: UIScreen.main.bounds.height * 0.033),
            toAuthenticationStackView.centerXAnchor.constraint(equalTo: centerXAnchor)
        ])

    }

    // Надпись "Нет аккаунта?"
    let toAuthenticationLabel: UILabel = {
        let toRegistrationLabel = UILabel()
        toRegistrationLabel.translatesAutoresizingMaskIntoConstraints = false
        toRegistrationLabel.text = "Не тот аккаунт?"
    ...
    override init(frame: CGRect) {
        ...
        addSubview(toAuthenticationStackView)
        toAuthenticationStackViewConstraints()

        toAuthenticationStackView.addArrangedSubview(toAuthenticationLabel)
        toAuthenticationStackView.addArrangedSubview(toAuthenticationActionLabel)
